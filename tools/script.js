...
function createToolCardHTML(tool) {
    ...
    // Includes Icon, Title, and the Description field.
    return `
...
function createSectionHTML(category, toolsHTML) {
    // Generates a section with the Category Header and the Grid of Tools
    return `
    <section>
...
function renderTools(filterText = '') {
    ...
    CATEGORIES.forEach((category) => {
        ...
        if (categoryTools.length > 0) {
            ...
            container.innerHTML += createSectionHTML(category, toolsHTML);
        }
    });
